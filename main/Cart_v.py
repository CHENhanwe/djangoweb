...
def cart_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/djangoqj7b0/cart/notify',
            #notify_url=''
        )
...
def cart_notify(request):
    if request.method in ["POST", "GET"]:
        ...
        return redirect('http://localhost:8080/djangoqj7b0/admin/dist/index.html#/cart')


def cart_groupby(request):
    '''
    ...
    if request.method in ["POST", "GET"]:
        ...
        # print(cart.getallcolumn(cart,cart))
        if __authSeparate__=="是":
            #print("req_dict==============>",req_dict)
            #如果当前表有userid字段,且通过身份认证
            if "userid"  in cart.getallcolumn(cart,cart) and request.session.get("params")!=None:
                #参数增加userid
                req_dict["userid"]=request.session.get("params").get("id")
        #如果当前表没有userid字段,且参数里有userid键值对
        if  "userid" not  in cart.getallcolumn(cart,cart) and "userid" in list(req_dict.keys()):
            #删除参数userid
            del req_dict["userid"]
        try:
            del req_dict["page"]
        ...
        #本次查询所有,求总记录数
        # alldata = cart.objects.filter(**req_dict). \
        ...
        datas = cart.objects.filter(**req_dict).\
...
def cart_deleterecords(request):
    '''
    按键值对参数添加删除记录
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code}
        req_dict = request.session.get("req_dict")
        error=cart.deletebyparams(cart,cart,req_dict)
        if error!=None:
            msg['code'] = crud_error_code
            msg['msg'] = error
        return JsonResponse(msg)
def cart_flist(request):
    '''
    查看所有开放的帖列表(无需登录)
    '''
    if request.method in ["POST", "GET"]:
