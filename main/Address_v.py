...
def address_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/djangoqj7b0/address/notify',
            #notify_url=''
        )
...
def address_notify(request):
    if request.method in ["POST", "GET"]:
        ...
        return redirect('http://localhost:8080/djangoqj7b0/admin/dist/index.html#/address')


def address_groupby(request):
    '''
    ...
    if request.method in ["POST", "GET"]:
        ...
        # print(address.getallcolumn(address,address))
        if __authSeparate__=="是":
            #print("req_dict==============>",req_dict)
            #如果当前表有userid字段,且通过身份认证
            if "userid"  in address.getallcolumn(address,address) and request.session.get("params")!=None:
                #参数增加userid
                req_dict["userid"]=request.session.get("params").get("id")
        #如果当前表没有userid字段,且参数里有userid键值对
        if  "userid" not  in address.getallcolumn(address,address) and "userid" in list(req_dict.keys()):
            #删除参数userid
            del req_dict["userid"]
        try:
            del req_dict["page"]
        ...
        #本次查询所有,求总记录数
        # alldata = address.objects.filter(**req_dict). \
        ...
        datas = address.objects.filter(**req_dict).\
...
def address_list_id(request,id_):
    '''
    查看主贴和所有回帖内容(无需登录)
    '''
    if request.method in ["POST", "GET"]:
        ...
        datas = address.retrieve(address, address)
