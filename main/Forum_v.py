...
def forum_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/djangoqj7b0/forum/notify',
            #notify_url=''
        )
...
def forum_notify(request):
    if request.method in ["POST", "GET"]:
        ...
        return redirect('http://localhost:8080/djangoqj7b0/admin/dist/index.html#/forum')


def forum_groupby(request):
    '''
    ...
    if request.method in ["POST", "GET"]:
        ...
        # print(forum.getallcolumn(forum,forum))
        if __authSeparate__=="是":
            #print("req_dict==============>",req_dict)
            #如果当前表有userid字段,且通过身份认证
            if "userid"  in forum.getallcolumn(forum,forum) and request.session.get("params")!=None:
                #参数增加userid
                req_dict["userid"]=request.session.get("params").get("id")
        #如果当前表没有userid字段,且参数里有userid键值对
        if  "userid" not  in forum.getallcolumn(forum,forum) and "userid" in list(req_dict.keys()):
            #删除参数userid
            del req_dict["userid"]
        try:
            del req_dict["page"]
        ...
        #本次查询所有,求总记录数
        # alldata = forum.objects.filter(**req_dict). \
        ...
        datas = forum.objects.filter(**req_dict).\
...
def forum_deleterecords(request):
    '''
    按键值对参数添加删除记录
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code}
        req_dict = request.session.get("req_dict")
        error=forum.deletebyparams(forum,forum,req_dict)
        if error!=None:
            msg['code'] = crud_error_code
            msg['msg'] = error
        return JsonResponse(msg)
